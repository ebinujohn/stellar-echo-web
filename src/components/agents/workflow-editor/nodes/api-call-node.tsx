...
export const ApiCallNode = memo(({ data, selected }: NodeProps<WorkflowNodeData>) => {
  ...
  const displayUrl = apiCall?.url
    ? apiCall.url.length > 50
      ? `${apiCall.url.substring(0, 50)}...`
      : apiCall.url
    : 'No URL configured';
  ...
  return (
    <div
      className={cn(
        ...
        selected
          ...
          : hasUrl
            ? 'border-green-500/50 hover:border-green-500 border-t-green-500'
    ...
    >
      ...
      {/* Header */}
      <div className="border-b border-border bg-green-500/20 px-3 py-2">
      ...
      <div className="p-3 space-y-2">
        ...
        <TransitionHandleRow transitions={data.transitions || []} color="green" />
      </div>
    </div>
  );
});
