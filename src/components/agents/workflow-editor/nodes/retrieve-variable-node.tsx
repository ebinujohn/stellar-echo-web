...
export const RetrieveVariableNode = memo(({ data, selected }: NodeProps<WorkflowNodeData>) => {
  ...
  return (
    <div
      className={cn(
        ...
        selected
          ...
          : isValid
            ? 'border-amber-500/50 hover:border-amber-500 border-t-amber-500'
    ...
    >
      ...
      {/* Header */}
      <div className="border-b border-border bg-amber-500/20 px-3 py-2">
      ...
      <div className="p-3 space-y-2">
        ...
        {variableNames.length > 0 ? (
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              {variableNames.slice(0, 6).map((varName) => (
        ...
        <TransitionHandleRow transitions={data.transitions || []} color="amber" />
      </div>
    </div>
  );
});
